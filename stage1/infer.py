...
class OverrideConfig(FairseqDataclass):
    ...
    max_sample_size: int = field(default=700)
...
class InferenceProcessor:
    ...
    def __init__(self, cfg: InferConfig) -> None:
        ...
        models, saved_cfg, self.task = checkpoint_utils.load_model_ensemble_and_task([cfg.common_eval.path])

        self.task = tasks.setup_task(saved_cfg.task)
        ...
        self.task.cfg.noise_prob = cfg.override.noise_prob
        self.task.cfg.noise_snr = cfg.override.noise_snr
        self.task.cfg.noise_wav = cfg.override.noise_wav
        self.task.cfg.modalities = cfg.override.modalities
        self.task.cfg.max_sample_size = cfg.override.max_sample_size
        #self.task.cfg.max_trim_sample_size = cfg.override.max_sample_size
        saved_cfg.task.max_sample_size = cfg.override.max_sample_size
        #saved_cfg.task.max_trim_sample_size = cfg.override.max_sample_size
        
        if cfg.override.data is not None:
            self.task.cfg.data = cfg.override.data
        if cfg.override.label_dir is not None:
            self.task.cfg.label_dir = cfg.override.label_dir
    ...
    def process_sentence(
        ...
        hyp_pieces = self.tgt_dict.string(hypo.int().cpu())
        hyp_words = hyp_pieces.replace(" ", "").replace("|", " ").strip()
    ...
    def process_sample(self, sample: Dict[str, Any]) -> None:
        ...
        #import pdb;pdb.set_trace()
        _, roman_toks = self.models[0].extract_features(**sample['net_input'])
        roman_toks = roman_toks.argmax(-1)
        roman_toks_list = []
        
        for i in range(roman_toks.size(0)):
            roman_tok = roman_toks[i].unique_consecutive()
            roman_tok = roman_tok[roman_tok != 0]
...
def get_wer_file(cfg: InferConfig) -> Path:
    ...
    if cfg.decoding.results_path is not None:
        os.makedirs(cfg.decoding.results_path, exist_ok=True)
